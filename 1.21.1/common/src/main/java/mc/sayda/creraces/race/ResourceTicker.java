    ...
    public static void tick(Player player) {
        ...
        if (interval >= 0 && player.level().isDay()
            ...
            if (!immune) {
                ...
                var sunProtEnch = mc.sayda.creraces.registry.ModEnchantments.get(player.level(),
                        mc.sayda.creraces.registry.ModEnchantments.SUN_PROTECTION);
                ...
                if (!headItem.isEmpty()) {
                    if (interval > 0 && player.tickCount % interval == 0 && headItem.isDamageableItem() && !hasProtection) {
                        ...
                        if (headItem.getDamageValue() >= headItem.getMaxDamage()) {
                            player.onEquippedItemBroken(headItem.getItem(), net.minecraft.world.entity.EquipmentSlot.HEAD);
                            player.setItemSlot(net.minecraft.world.entity.EquipmentSlot.HEAD,
                                    net.minecraft.world.item.ItemStack.EMPTY);
                        }
                    }
                } else if (player.tickCount % 20 == 0) {
                    player.igniteForSeconds(mc.sayda.creraces.config.CreRacesConfig.SUNLIGHT_BURN_SECONDS.get().floatValue());
                }
            }
        }
