...
public class SocialPassivesHelper {
    ...
    private static TagKey<EntityType<?>> getOrCreateTag(@javax.annotation.Nonnull String path) {
        return TAG_CACHE.computeIfAbsent(path, p -> TagKey.create(Registries.ENTITY_TYPE, ResourceLocation.parse(p)));
    }
     ...
     * #-prefixed entity type tag reference (with a per-instance fallback for undead-race players).
     */
    private static boolean matchesEntry(LivingEntity entity, EntityType<?> entityType, ResourceLocation entityId,
        ...
        if (spec.startsWith("#")) {
            ...
            // Fallback for the undead tag: catches undead-race players, who are never
            // in the vanilla entity type tag but return true here via LivingEntityMixin.
            if (tagPath.equals("minecraft:undead") && entity.isInvertedHealAndHarm()) {
                return true;
            }
            return false;
        } else {
