...
public class SpiritRealmRenderer {
    ...
    private static void renderMoonUnit(PoseStack poseStack, float partialTicks, boolean isSecond, boolean isMirror) {
        ...
        if (isMirror) {
            ...
            bufferBuilder.addVertex(matrix, -moonSize, yPos, -moonSize).setUv(u1, v1);
            bufferBuilder.addVertex(matrix, moonSize, yPos, -moonSize).setUv(u2, v1);
            bufferBuilder.addVertex(matrix, moonSize, yPos, moonSize).setUv(u2, v2);
            bufferBuilder.addVertex(matrix, -moonSize, yPos, moonSize).setUv(u1, v2);
        } else {
