...
public class CommandingStaffItem extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(@Nonnull Level level, @Nonnull Player player,
        ...
        if (player.isShiftKeyDown()) {
            ...
            mc.sayda.creraces.util.ItemNbt.mutate(stack, t -> t.putString("PendingMode", nextMode));
        ...
        if (tag.contains("PendingMode")) {
            ...
            mc.sayda.creraces.util.ItemNbt.mutate(stack, t -> {
                t.putString("CommandMode", pending);
                t.remove("PendingMode");
            });
            tag = mc.sayda.creraces.util.ItemNbt.get(stack);
        }
        ...
        if (commandMode.equals("attack")) {
            ...
            if (entityHit != null && entityHit.getEntity() instanceof LivingEntity target) {
                if (!level.isClientSide) {
                    mc.sayda.creraces.util.ItemNbt.mutate(stack, t -> t.putUUID("CommandTarget", target.getUUID()));
                    mc.sayda.creraces.util.ItemNbt.mutate(stack, t -> t.remove("CommandPos"));
                    player.displayClientMessage(
        ...
        if (commandMode.equals("move") || commandMode.equals("attack")) {
            ...
            if (blockHit.getType() == HitResult.Type.BLOCK) {
                ...
                if (!level.isClientSide) {
                    ...
                    mc.sayda.creraces.util.ItemNbt.mutate(stack, t -> t.put("CommandPos", posTag));
                    mc.sayda.creraces.util.ItemNbt.mutate(stack, t -> t.remove("CommandTarget"));
    ...
    @Override
    public void appendHoverText(@Nonnull ItemStack stack, net.minecraft.world.item.Item.TooltipContext level, @Nonnull List<Component> tooltip,
            @Nonnull TooltipFlag flag) {
        tooltip.add(Component.translatable("item.creraces.commanding_staff.desc").withStyle(ChatFormatting.GRAY));
        String commandMode = mc.sayda.creraces.util.ItemNbt.get(stack).contains("CommandMode")
                ? mc.sayda.creraces.util.ItemNbt.get(stack).getString("CommandMode")
                : "follow";
