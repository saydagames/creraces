        ...
        DataUtils.getVariables(player).ifPresent(vars -> {
            ...
            if (soggyWings && (soggyCondition == null || soggyCondition.evaluate(player, null, null, null))) {
                ...
                if (shouldBeSoggy) {
                    ...
                    var soggyEffect = mc.sayda.creraces.registry.ModMobEffects.SOGGY;
            ...
            var soggyEffectRef = mc.sayda.creraces.registry.ModMobEffects.SOGGY;
            boolean isSoggy = soggyWings && soggyEffectRef != null && player.hasEffect(soggyEffectRef);
            var brokenWingsRef = mc.sayda.creraces.registry.ModMobEffects.BROKEN_WINGS;
            boolean hasBrokenWings = brokenWingsRef != null && player.hasEffect(brokenWingsRef);
            boolean resourceOk = resource == ResourceType.NONE || currentResource >= evaluatedDrain;
            boolean canFly = conditionMet && resourceOk && !isSoggy && !hasBrokenWings;
            ...
            if (canFly) {
                ...
                if (!player.isCreative() && !player.isSpectator()) {
                    ...
                    // On Fail Logic
                    ResourceLocation failId = ResourceLocation.fromNamespaceAndPath(java.util.Objects.requireNonNull(traitId.getNamespace()),
                            java.util.Objects.requireNonNull(traitId.getPath()) + "_failed");
                    boolean alreadyFailed = vars.getPersistentState(failId) > 0;
                    if (conditionMet && (!resourceOk || isSoggy || hasBrokenWings) && !alreadyFailed) {
            ...
            if (canFly) {
                ResourceLocation failId = ResourceLocation.fromNamespaceAndPath(java.util.Objects.requireNonNull(traitId.getNamespace()), java.util.Objects.requireNonNull(traitId.getPath()) + "_failed");
                vars.setPersistentState(failId, 0.0);
            }
    ...
    public static void register() {
        TraitRegistry.register(ResourceLocation.fromNamespaceAndPath(CreRaces.MODID, "flight"), json -> {
            ...
            String traitName = json.has("name") ? json.get("name").getAsString()
                    : "flight_" + resStr.toLowerCase() + "_" + Math.abs(drainRate.toString().hashCode());
