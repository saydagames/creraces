...
public final class GuildReceptionistOfferSync {
    ...
    /**
     * Both readers go through the QUEST_GRADE component when the cost has no CUSTOM_DATA left,
     * which is the case for every offer restored from villager save data (ItemCost rebuilds its
     * display stack from item/count/components). Without that fallback nothing here recognises
     * an existing tier and sync() appends a second full set of 5 on the next interaction.
     */
