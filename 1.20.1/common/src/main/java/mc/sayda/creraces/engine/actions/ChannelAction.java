...
public class ChannelAction implements ActionRegistry.RaceAction {
    ...
    public static final ResourceLocation ID = new ResourceLocation("creraces", "channel");
        ...
        ActionRegistry.register(ID, json -> {
            ...
            if (json.has("during_effects")) {
                for (JsonElement e : json.getAsJsonArray("during_effects")) {
                    ...
                    if (loc != null) {
                        MobEffect eff = net.minecraft.core.registries.BuiltInRegistries.MOB_EFFECT.get(loc);
                        if (eff != null) effects.add(new ChannelingManager.DuringEffect(eff, amp, dur));
                    }
                }
            }
